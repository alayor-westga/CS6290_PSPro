...
Scenario: Login as supervisor
	...
	When click on Login
	Then the user full name "Super Visor (s-001)" is shown in the new complaint window

Scenario: Login as investigator
	Given username is "i-001"
	And password is "4567"
	When click on Login
	Then the user full name "Investi Gator (i-001)" is shown in the investigator dashboard window
